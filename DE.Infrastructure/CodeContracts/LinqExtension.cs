        ...
        private static LambdaExpression GenerateSelector<TEntity>(String propertyName, out Type resultType) where TEntity : class
        {
            ...
            if (propertyName.Contains('.'))
            {
                ...
                for (int i = 1; i < childProperties.Length; i++)
                {
                    ...
                    if (property == null)
                        throw new ApplicationException(string.Format("Could not find a property named {0} in the given object", childProperties[i]));
            ...
            else
            {
                ...
                if (property == null)
                    throw new ApplicationException(string.Format("Could not find a property named {0} in the given object", propertyName));
                propertyAccess = Expression.MakeMemberAccess(parameter, property);
            }
